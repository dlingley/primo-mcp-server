...
def strip_html(text: str) -> str:
    """Strip HTML tags and unescape entities in LibGuides rich-text fields."""
    if not text:
        return ""
...
class SpringshareClient:
    ...
    def __init__(self, http_client: httpx.AsyncClient, config: SpringshareConfig):
        ...
        # The A-Z list is curated and changes rarely, so one fetch serves
        # many searches. Entries are cached for config.az_cache_ttl seconds.
        self._az_cache: list[dict[str, Any]] | None = None
        self._az_cache_expires_at: float = 0.0
    ...
    async def get_databases(self) -> list[dict[str, Any]]:
        """Fetch all A-Z databases with expanded subjects, cached with a TTL."""
        now = time.monotonic()
        if self._az_cache is not None and now < self._az_cache_expires_at:
            return self._az_cache
        ...
        try:
            ...
            resp.raise_for_status()
            databases = resp.json()
            self._az_cache = databases
            self._az_cache_expires_at = now + self.config.az_cache_ttl
            return databases
        except httpx.HTTPStatusError as e:
    ...
    async def search_databases(self, query: str) -> list[dict[str, Any]]:
        """Search the cached A-Z list locally.

        Returns matching databases sorted by match quality then name.
        Cached entries are never mutated.
        """
        ...
        for db in databases:
            ...
            if score > 0:
                scored.append((score, name_lower, db))

        # Sort by score (highest first), then by name alphabetically
        scored.sort(key=lambda item: (-item[0], item[1]))
        return [db for _, _, db in scored]
